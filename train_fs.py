    ...
    *,
    ...
    for run in range(num_runs):
        ...
        if not Constants["DEACTIVATE_WANDB"]:
            print(f"\n\n\n\nLogging to wandb: {run}")
            assert isinstance(run, int), f"Expected run to be an integer, got {type(run)}"
...
def filter_per_number_samples_one_set(X, Y, samples_per_class, subset):

    subset_classes = np.unique(Y)
    reduced_subset_classes = [x for x in subset_classes if (Y == x).sum() >= samples_per_class]
    
    print("\n\nREDUCED", subset, len(reduced_subset_classes), "from", len(subset_classes))
    
    # Filter X and Y
    mask = np.isin(Y, reduced_subset_classes)
    X_reduced = X[mask]
    Y_reduced = Y[mask]
...
def filter_per_number_samples(XTrain, YTrain, XTest, YTest, samples_per_class):
    # Exclude classes with less than samples_per_class
    if Constants["EXCLUDE_CLASSES_WITH_LESS_SAMPLES"]:
        XTrain, YTrain = filter_per_number_samples_one_set(XTrain, YTrain, samples_per_class, "Train")
        XTest, YTest = filter_per_number_samples_one_set(XTest, YTest, samples_per_class, "Test")
